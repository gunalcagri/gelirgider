export async function getOrder(id: string) {
  return (await getExpenses()).find((order) => order.id.toString() === id)!
}

export async function getRecentOrders() {
  return (await getExpenses()).slice(0, 10)
}
...
export async function getEventOrders(id: string) {
  return (await getExpenses()).filter((order) => order.event.id.toString() === id)
}
